...
def main():
    try:
        ...
        control = PID(target, 0.5, 0.0, 1.0, 0.2)
        ...
        with open('temp_data_multiple.csv', 'r', newline='') as csvfile:
            reader = csv.reader(csvfile)
            #start_time = time.time()-line+number_of_thermometers to offset the wait time
            for line in reader:
                pass
            if line:
                print(line)
                try:
                    start_time = time.time()-float(line[0])-loop_value
                except:
                    print("No data currently in file.")
